...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, epoch, model_save_path): # this one is better for visualization purpose, i.e., save feature maps during training and visualize
    '''if args.trainRT_net:
    ...
    #model_RT_relative = models.resnet50(pretrained=True)
    ...
    '''for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src_) in enumerate(train_dataloader_):
        ...
        rot_out, tra_out, x_conv1, x_conv2, x_linear1, x_linear2 = model_RT_relative(ref_imgs.cuda(), src_imgs.cuda())
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(pos_out.shape, ori_out.shape, pos_out, ori_out))
        print("conv1_size(0) : {}, conv1_size(1) : {}".format(x_conv1.size(0), x_conv1.size(1)))
        #print("conv2_size(0) : {}, conv2_size(1) : {}".format(x_conv2.size(0), x_conv2.size(1)))
        #print('filename_to_save_src_: ', filename_to_save_src_[0])
        print('filename_to_save_ref_: ', filename_to_save_ref)
        #quit()

        if 0:
            filename_to_save_ref = filename_to_save_ref[0]
            #filename_to_save_src_ = filename_to_save_src_[0]
            for j in range(batch_size): #number of B
                for l in range(n_view - 1): # number of V
                    for k_ in range(x_conv1.size(1)): #number of channels
                        print("j = {}, l = {}, k = {}".format(j,l, k_))
                        x_conv1_img = x_conv1[j*(n_view-1) + l, k_].detach().cpu().numpy().astype(np.float32)
                        x_linear1_img = x_linear1[j * (n_view - 1) + l].detach().cpu().numpy().astype(np.float32).reshape(32,32)
                        x_conv2_img = x_conv2[j*(n_view-1) + l, k_].detach().cpu().numpy().astype(np.float32)
                        x_linear2_img = x_linear2[j * (n_view - 1) + l].detach().cpu().numpy().astype(np.float32).reshape(32,32)
                        if os.path.exists('./' + outdir + '/epoch_' + str(epoch) + set):
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + str(k_) + '_R.pfm',x_conv1_img)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + '_R_linear.pfm', x_linear1_img)
                            filename_to_save_src_2 = filename_to_save_src_[l]
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_src_2[j][:-10] + '_con_' + str(k_) + '_S.pfm', x_conv2_img)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_src_2[j][:-10] + '_con_' + '_S_linear.pfm', x_linear2_img)
                            del x_conv1_img, x_conv2_img, x_linear1_img, x_linear2_img
                        else:
...
def train_RTNetKP(model_RT_relative, optimizer_RTNet, schedular_RTNet, epoch, model_save_path): # this one is better for visualization purpose, i.e., save feature maps during training and visualize
    if args.trainRT_net:
        model_RT_relative.train()
        # criterion_RT = nn.MSELoss(reduction='mean')
        criterion_RT = nn.L1Loss(reduction='sum')
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src_,_, _) in enumerate(train_dataloader_):
        #print("Filename : {} \n".format(filename_to_save_))
        #print("rel_quat_ : {} \n".format(rel_quat_))
        ...
        optimizer_RTNet.zero_grad()
        #rot_out, tra_out, x_conv1, x_conv2, x_linear1, x_linear2 = model_RT_relative(ref_imgs.cuda(), src_imgs.cuda())
        rot_out, tra_out, x_conv1, x_conv2, x_conv_sim, x_linear1 = model_RT_relative(ref_imgs.cuda(), src_imgs.cuda()) #--> relRT25
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(pos_out.shape, ori_out.shape, pos_out, ori_out))
        print("conv1_size(0) : {}, conv1_size(1) : {}".format(x_conv1.size(0), x_conv1.size(1)))
        #print("conv2_size(0) : {}, conv2_size(1) : {}".format(x_conv2.size(0), x_conv2.size(1)))
        #print('filename_to_save_src_: ', filename_to_save_src_[0])
            ...
            for j in range(batch_size): #number of B
                for l in range(n_view - 1): # number of V
                    for k_ in range(x_conv1.size(1)): #number of channels
                        ...
                        x_linear1_img = x_linear1[j * (n_view - 1) + l].detach().cpu().numpy().astype(np.float32).reshape(32,32)
                        x_conv2_img = x_conv2[j*(n_view-1) + l, k_].detach().cpu().numpy().astype(np.float32)
                        x_conv_sim_img = x_conv_sim[j * (n_view - 1) + l, k_].detach().cpu().numpy().astype(np.float32)
                        #x_linear2_img = x_linear2[j * (n_view - 1) + l].detach().cpu().numpy().astype(np.float32).reshape(32,32)
                        if os.path.exists('./' + outdir + '/epoch_' + str(epoch) + set):
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + str(k_) + '_R.pfm',x_conv1_img)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + '_R_linear.pfm', x_linear1_img)
                            filename_to_save_src_2 = filename_to_save_src_[l]
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_src_2[j][:-10] + '_con_' + str(k_) + '_S.pfm', x_conv2_img)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_src_2[j][:-10] + '_con_' + str(k_) + '_sim.pfm', x_conv_sim_img)
                            #write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_src_2[j][:-10] + '_con_' + '_S_linear.pfm', x_linear2_img)
                            del x_conv1_img, x_conv2_img, x_linear1_img, x_conv_sim_img #, x_linear2_img
                        else:
                            os.mkdir('./' + outdir + '/epoch_' + str(epoch) + set)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + str(k_) + '_R.pfm',x_conv1_img)
                            write_pfm('./' + outdir + '/epoch_' + str(epoch) + set + filename_to_save_ref[j][:-10] + '_con_' + '_R_linear.pfm', x_linear1_img)
...
def test_RTNet(model_RT):
    avgMSEloss_RTnet = 0
    criterion_RT = nn.L1Loss()
    model_RT.eval()
    ...
    for batch_idx, (
            imgs, depth_gt, depth_values, mask, imgs_RT, filename_to_save, proj_mats, gt_quaternion) in enumerate(
        test_dataloader_):
        ...
        # pos_out, ori_out = model(images1)
        B, V, C, H, W = imgs_RT.shape  # Zahid for RT B - batch size, V - no of views
        imgs_RT = imgs_RT.reshape(B * V, 3, H, W)  # --> pass the view images as batch images to RT network
        # print("B={}, V={}, C={}, H={}, W={}".format(B, V, C, H, W))

        with torch.no_grad():
            pos_out, ori_out = model_RT(imgs_RT.cuda())
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(pos_out.shape, ori_out.shape, pos_out, ori_out))

        with torch.no_grad():
            ori_out = F.normalize(ori_out, p=2, dim=1)

        pred = torch.cat((ori_out, pos_out), dim=1)

        # loss_rtnet = criterion_RT(pred, gt_poses)
        # print("size of pred = {}, pred = {}".format(pred.shape, pred))

        # print("size of gt_qua = {}, gt_qua = {}".format((gt_quaternion[0].shape), torch.cat(gt_quaternion[0], gt_quaternion[1], gt_quaternion[0])))
        gt_quaternion = gt_quaternion.reshape(B * V, 7)
        ...
        avgMSEloss_RTnet += criterion_RT(pred, gt_quaternion.cuda())
        # print("Loss = {}".format(loss_rtnet))
        # quit()
...
if args.trainRT_net:
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        #train_RTNetKP(model_RT_relative, optimizer_RTNet, schedular_RTNet, epoch, model_save_path)
        train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, epoch, model_save_path)
