...
from datasets.dtu_both_relative7Scenes import SevenScenes # --> new conversion
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
                ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, depth) in enumerate(
                train_dataloader_):
            ...
            for i in range(B_ * V_):
                ...
                points2_proj_GT = inverse_project(intrinsics, rel_quat_[i, :4], rel_quat_[i, 4:], depth[math.floor(i / V_)],
                                                  points1)
                points2_proj_pre = inverse_project(intrinsics, pred[i, :4].cpu().detach(), pred[i, 4:].cpu().detach(),
                ...
                for j in range(len(points1)):
                    if(points2_proj_GT[j,0]>0 and points2_proj_pre[j,0] >0 and points2_proj_GT[j,1]>0 and points2_proj_pre[j,1]>0 and depth[math.floor(i / V_)][points1[j,1], points1[j,0]] > 0\
                        and points2_proj_GT[j, 0] < 640 and points2_proj_GT[j, 1] < 480 ):
        ...
        if (loss_rtnet < old_loss):
            ...
            print('Saving models at Epoch : {} out of {}, batch : {} out of {}, RT loss : {}, Lr : {}'.format(epoch, args.num_epochs,
                                                                                                    batch_idx,
                                                                                                    len(train_dataloader_),
                                                                                                    loss_rtnet, lr))
