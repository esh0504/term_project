...
from datasets.dataset_DTU_new import DTUDataset # --> new conversion, includes reading KPS files
...
# models
from models.siamese_Zahid import SiameseNet
...
'''
def test_dataloader():
    test_dataset = DTUDataset(root_dir=args.root_dir, split='test', transforms=val_augs)
    if args.num_gpus > 1:
...
def train_dataloader():
    train_dataset = DTUDataset(root_dir=args.root_dir, split='train', transforms=train_augs)
    if args.num_gpus > 1:
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for train_sample in tqdm(train_dataloader_):
            ...
            mat_out, x_conv1, x_conv2 = model_RT_relative(train_sample['img1'].cuda(), train_sample['img2'].cuda())

            R1_gt = train_sample['R1']
            R2_gt = train_sample['R2']
            R3_gt = train_sample['R3']
            T1_gt = train_sample['T1']
            R4_gt = train_sample['R4']
            R5_gt = train_sample['R5']
            R6_gt = train_sample['R6']
            T2_gt = train_sample['T2']
            R7_gt = train_sample['R7']
            R8_gt = train_sample['R8']
            R9_gt = train_sample['R9']
            T3_gt = train_sample['T3']

            #print('matout : {}'.format(mat_out))
...
if args.trainRT_net:
    ...
    #sum_mode = False
    model_RT_relative = SiameseNet(base_model_RT, args.dropout_rate, args.bayesian)
