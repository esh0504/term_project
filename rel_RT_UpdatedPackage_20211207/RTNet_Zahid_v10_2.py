...
from datasets.dataset_DTU_new import DTUDatasetClassify # --> new conversion, includes reading KPS files
...
# models
from models.siamese_Zahid import SiameseNetClassify
...
'''
def test_dataloader():
    test_dataset = DTUDatasetClassify(root_dir=args.root_dir, split='test', transforms=val_augs)
    if args.num_gpus > 1:
...
def train_dataloader():
    train_dataset = DTUDatasetClassify(root_dir=args.root_dir, split='train', transforms=train_augs)
    if args.num_gpus > 1:
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    if args.trainRT_net:
        ...
        criterion_RT = nn.CrossEntropyLoss(reduction='sum')
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for train_sample in tqdm(train_dataloader_):
            ...
            out, x_conv1, x_conv2 = model_RT_relative(train_sample['img1'].cuda(), train_sample['img2'].cuda())

            gt_labels = train_sample['gt_labels']
            ...
            #print('matout - R1 : {}'.format(w1 * criterion_RT(mat_out[:, 0], R1_gt.cuda())))
            #quit()
            # print('R1 : {}'.format(R1_gt))

            if(0):
                loss_rtnet = w1 * criterion_RT(mat_out[:, 0], R1_gt.cuda()) + w1 * criterion_RT(mat_out[:, 1], R2_gt.cuda()) + w1 * criterion_RT(mat_out[:, 2], R3_gt.cuda()) + w2 * criterion_RT(mat_out[:, 3],
                                                                             T1_gt.cuda()) + w1 * criterion_RT(mat_out[:, 4], R4_gt.cuda()) + w1 * criterion_RT(mat_out[:, 5], R5_gt.cuda()) + w1 * criterion_RT(
                    mat_out[:, 6], R6_gt.cuda()) + w2 * criterion_RT(mat_out[:, 7], T2_gt.cuda()) + w1 * criterion_RT(mat_out[:, 8], R7_gt.cuda()) + w1 * criterion_RT(mat_out[:, 9], R8_gt.cuda()) + w1 * criterion_RT(
                    mat_out[:, 10], R9_gt.cuda()) + w2 * criterion_RT(mat_out[:, 11], T3_gt.cuda())
            else:
                loss_rtnet = criterion_RT(out, gt_labels.cuda().long())
...
def test_RTNet(model_RT_relative, exp_name, start_epoch):
    ...
    for test_sample in tqdm(test_dataloader_):
        ...
        gt_labels = test_sample['gt_labels']
        fname1 = test_sample['fnames1']
        fname2 = test_sample['fnames2']
        ...
        with torch.no_grad():
            ...
            out, x_conv1, x_conv2 = model_RT_relative(test_sample['img1'].cuda(),
                                                                   test_sample['img2'].cuda())
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(tra_out.shape, rot_out.shape, tra_out, rot_out))
        # print("size of pos_out={}, ori_out={}".format(tra_out,rot_out))

        _, prediction = torch.max(out.data, 1)
        # prediction = prediction.cpu().numpy() #pytorch0.4.1
        # pred_ = prediction
        pred_ = prediction.cpu().numpy()  # pytorch 1.0.0
        labels_ = gt_labels.cpu().data.numpy()
        # test_acc += torch.sum(prediction == labels.data).item() # pytorch 0.4.1
        test_acc += torch.sum(prediction == gt_labels.cuda().data).item()  # pytorch 1.0.0

        '''print('out = {}'.format(out))
        print('prediction = {}'.format(prediction))
        print('pred_ = {}'.format(pred_))
        print('labels_ = {}'.format(labels_))
        print('gt_labels = {}'.format(gt_labels.cuda().data))
        ...
        for i in range(V):
            fp_gt.write(fname1[i] + ' ' + fname2[i] + ' ' + str(gt_labels[i]) + '\n')
            fp_pr.write(fname1[i] + ' ' + fname2[i] + ' ' + str(pred_[i]) + '\n')
        # print("Loss = {}".format(loss_rtnet))
        # quit()
    ...
    test_acc = test_acc / len(test_dataloader_)  # 17512 # 13955 # 17512 # 54215 # 3384 #10000
    print('Final Acc. on Test Set is : {}'.format(test_acc))
    fp_gt.close()
    fp_pr.close()
...
if args.trainRT_net:
    ...
    #sum_mode = False
    model_RT_relative = SiameseNetClassify(base_model_RT, args.dropout_rate, args.bayesian)
