...
# models
from models.hourglass_Zahidv4 import HourglassNet
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            #rot_out = F.normalize(rot_out, p=2, dim=1) # --> for version relRT9, we commented the normalization layer
            rot_out = F.normalize(rot_out, p=2, dim=1) # --> for version relRT21 we put back the normalization layer
            ...
            loss_rtnet = w1 * criterion_RT(pred[:, 0], rel_quat_[:, 0].cuda()) + w1 * criterion_RT(pred[:, 1],rel_quat_[:,1].cuda()) + w1 * criterion_RT(
                pred[:, 2], rel_quat_[:, 2].cuda()) + w1 * criterion_RT(pred[:, 3],rel_quat_[:, 3].cuda()) + w2 * criterion_RT(
                pred[:, 4], rel_quat_[:, 4].cuda()) + w2 * criterion_RT(pred[:, 5],rel_quat_[:, 5].cuda()) + w2 * criterion_RT(
...
def test_MVSNet(model_RT_relative, model_MVSNet, epoch):
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        ...
        rot_out = F.normalize(rot_out, p=2, dim=1) # --> uncommented for relRT21
