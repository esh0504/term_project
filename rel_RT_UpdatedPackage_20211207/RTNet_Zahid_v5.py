...
def perc_loss(pred, gt):
    ...
    quit()'''
    return 150 * (torch.sum(abs(torch.div((pred[:, 0] - gt[:, 0].cuda()) , gt[:, 0].cuda()))))\
    + 200 *(torch.sum(abs(torch.div((pred[:, 1] - gt[:, 1].cuda()) , gt[:, 1].cuda()))))\
    + 200 *(torch.sum(abs(torch.div((pred[:, 2] - gt[:, 2].cuda()) , gt[:, 2].cuda()))))\
    + 200 *(torch.sum(abs(torch.div((pred[:, 3] - gt[:, 3].cuda()) , gt[:, 3].cuda()))))\
    + 100 *(torch.sum(abs(torch.div((pred[:, 4] - gt[:, 4].cuda()) , gt[:, 4].cuda()))))\
    + 100 *(torch.sum(abs(torch.div((pred[:, 5] - gt[:, 5].cuda()) , gt[:, 5].cuda()))))\
    + 100 *(torch.sum(abs(torch.div((pred[:, 6] - gt[:, 6].cuda()) , gt[:, 6].cuda()))))
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            rot_out = F.normalize(rot_out, p=2, dim=1) #--> putting back for relRT20,23 commented for relRT11
...
def test_RTNet(model_RT, exp_name, start_epoch):
    ...
    for batch_idx, (
        ...
        with torch.no_grad():
            rot_out = F.normalize(rot_out, p=2, dim=1) # --> uncommented for experiments above relRT19
...
def test_MVSNet(model_RT_relative, model_MVSNet, epoch):
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        ...
        rot_out = F.normalize(rot_out, p=2, dim=1) #--> uncommented for relRT20
