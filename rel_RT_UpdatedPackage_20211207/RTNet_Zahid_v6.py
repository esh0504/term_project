...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
                ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, depth, mask) in enumerate(
                train_dataloader_):
            ...
            for i in range(B_ * V_):
                #img_ = unpreprocess2_RT(src_imgs[i, 1, :, :]).cpu()  # batch 0, ref image, 1/4 scale
                #img_ = (src_imgs[:, :, :, :][i]).cpu()
                #transforms.Normalize(mean=[-0.485 / 0.229, -0.456 / 0.224, -0.406 / 0.225],
                 #                       std=[1 / 0.229, 1 / 0.224, 1 / 0.225])(img_)
                #src_imgs_SIFT = img_.numpy()
                #src_imgs_SIFT = np.transpose(src_imgs_SIFT, (1, 2, 0))
                #src_imgs_SIFT = np.subtract(src_imgs_SIFT, np.array((0.485 / 0.229, 0.456 / 0.224, 0.406 / 0.225)))
                #src_imgs_SIFT = np.divide(src_imgs_SIFT, np.array((0.229, 0.224, 0.225)))

                #src_imgs_SIFT = cv2.normalize(src_imgs_SIFT, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
                #src_imgs_gray = cv2.cvtColor(src_imgs_SIFT, cv2.COLOR_BGR2GRAY)
                #src_imgs_gray = src_imgs_gray.astype(np.int8)
                #print("shape = {}, Type = {} ".format(src_imgs_gray.shape, src_imgs_gray.dtype))
                #cv2.imwrite('src.png', src_imgs_gray)

                ref_imgs_ = ref_imgs[:, :, :, :][i].cpu()
                ref_imgs_SIFT = ref_imgs_.numpy()
                ref_imgs_SIFT = np.transpose(ref_imgs_SIFT, (1, 2, 0))
                ref_imgs_SIFT = np.subtract(ref_imgs_SIFT, np.array((0.485 / 0.229, 0.456 / 0.224, 0.406 / 0.225)))
                ref_imgs_SIFT = np.divide(ref_imgs_SIFT, np.array((0.229, 0.224, 0.225)))

                ref_imgs_SIFT = cv2.normalize(ref_imgs_SIFT, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
                ref_imgs_gray = cv2.cvtColor(ref_imgs_SIFT, cv2.COLOR_BGR2GRAY)
                #cv2.imwrite('ref.png', ref_imgs_gray)



                #src_imgs_SIFT = cv2.cvtColor(src_imgs_, cv2.COLOR_BGR2GRAY)
                #ref_imgs_SIFT = cv2.cvtColor(ref_imgs_, cv2.COLOR_BGR2GRAY)
                kp1, des1 = sift.detectAndCompute(ref_imgs_gray, None)
                #kp2, des2 = sift.detectAndCompute(src_imgs_gray, None)
                points1 = cv2.KeyPoint_convert(kp1)
                #points2 = cv2.KeyPoint_convert(kp2)

                if(isinstance(points1, tuple)):
                    points1 = np.array([[10,35], [11,45], [18,40], [25,50], [25, 20], [50, 20], [70, 20], [90, 20], [110, 20],\
                                        [140, 50], [25, 70], [25, 80], [70, 70], [110, 110]])

                points1[:, 0] = (points1[:, 0] * (160 / 224)) # --> rescale to 160x128 which is the size for depth images
                points1[:, 1] = (points1[:, 1] * (128 / 224)) # --> rescale to 160x128 which is the size for depth images

                if(points1.shape[0] < 4): #if the sift is not able to detect any point
                    points1 = np.array([[25,20],[140,50],[70,70],[110,110]])

                points2_proj = inverse_project(intrinsics, rel_quat_[i,:4], rel_quat_[i,4:], depth[math.floor(i/V_)], points1)

                '''fp = open("points1.txt", "w")
                fp1 = open("points2.txt", "w")
                for j in range(points1.shape[0]):
                    fp.write(str(points1[j][0]) + ' ' + str(points1[j][1]) + '\n')
                for j in range(len(points2_proj)):
                    fp1.write(str(points2_proj[j][0]) + ' ' + str(points2_proj[j][1]) + '\n')
                ...
                points1[:, 0] = (points1[:, 0] * (56 / 160))  # --> rescale to 56x56 which is the output feature size
                points1[:, 1] = (points1[:, 1] * (56 / 128))  # --> rescale to 56x56 which is the output feature size

                points2_proj = np.array(points2_proj)

                points2_proj[:, 0] = (
                            points2_proj[:, 0] * (56 / 160))  # --> rescale to 56x56 which is the output feature size
                points2_proj[:, 1] = (
                            points2_proj[:, 1] * (56 / 128))  # --> rescale to 56x56 which is the output feature size

                points1 = points1.astype(np.int32)
                points2_proj = points2_proj.astype(np.int32)
                ...
                mask_ = cv2.resize(mask_, (56, 56))

                for j in range(points1.shape[0] - 1):
                    if (points1[j][0] >= boundary_away and points1[j][0] < (w - boundary_away) and points1[j][
                        1] >= boundary_away and points1[j][1] < (h - boundary_away) and mask_[points1[j,1], points1[j,0]] > 0 \
                            and points2_proj[j][0] >= 0 and points2_proj[j][0] < (w) and points2_proj[j][
                        1] >= 0 and points2_proj[j][1] < (h) and (np.linalg.norm(points1[j] - points1[j+1])) > th):# only write points that are not close to boundary of image and lie on the object (mask)

                        new_ref_kps.append(points1[j])
                        new_src_kps.append(points2_proj[j])

                '''fp = open("points"+str(i)+"_ref.txt", "w")
                fp1 = open("points"+str(i)+"_src.txt", "w")
                for j in range(len(new_ref_kps)):
                    fp.write(str(new_ref_kps[j][0]) + ' ' + str(new_ref_kps[j][1]) + '\n')
                    fp1.write(str(new_src_kps[j][0]) + ' ' + str(new_src_kps[j][1]) + '\n')
               ...
               #quit()'''
                ...
                src_kps.append(new_src_kps)
                ref_kps.append(new_ref_kps)
            ...
            for i in range(B_ * V_):
                ...
                if(stride > 0):
                    for j in range(4): # use only 4 points to compute the loss
                        ...
                        loss_feat += torch.sum(abs((x_conv1[i,:,ref_kps[i][j*stride][1],ref_kps[i][j*stride][0]]) - (x_conv2[i,:,src_kps[i][j*stride][1], src_kps[i][j*stride][0]])))
                        cnt = cnt + 1
                        #print("Loss Feat : {}".format(loss_feat))
                else:
                    stride = 1
                    for j in range(len(ref_kps[i])): # use only 4 points to compute the loss
                        #print('ref point i={}, j={}, = ({},{})'.format(i,j, ref_kps[i][j*stride][1],ref_kps[i][j*stride][0]))
                        #print('src point i={}, j={}, =({},{})'.format(i, j, src_kps[i][j * stride][1], src_kps[i][j * stride][0]))
                        loss_feat += torch.sum(abs((x_conv1[i,:,ref_kps[i][j*stride][1],ref_kps[i][j*stride][0]]) - (x_conv2[i,:,src_kps[i][j*stride][1], src_kps[i][j*stride][0]])))
                        cnt = cnt + 1
            ...
            loss_feat /= cnt # --> must use average loss bcx for every sample, the number of points are different
            #print("Final Loss Feat : {}".format(loss_feat))
            #quit()

            #rot_out = F.normalize(rot_out, p=2, dim=1)
            rot_out = F.normalize(rot_out, p=2, dim=1) # --> uncommented for relRT23
            ...
            # (loss_rtnet+(.1*loss_feat)).backward()
...
def test_MVSNet(model_RT_relative, model_MVSNet, epoch):
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        ...
        #rot_out = F.normalize(rot_out, p=2, dim=1)
        rot_out = F.normalize(rot_out, p=2, dim=1) #uncomment for relRT23
