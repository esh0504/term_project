...
from models.hourglass_Zahidv6_Res50_featExt import HourglassNet
from models.hourglass_Zahidv6_Res50_Regress import RegressNet
...
def save_models(model, optimizer_RTRegNet, epoch, save_dir):
    torch.save(model.state_dict(), "{}/FeatExt_ckpts_{}.model".format(save_dir, epoch))
    torch.save(optimizer_RTRegNet.state_dict(), "{}/Regress_ckpts_{}.model".format(save_dir, epoch))
...
def configure_optimizers_RTNet(model_RtNet, model_RtRegNet):
    ...
    optimizer_RTRegNet = get_optimizer(args, model_RtRegNet)
    scheduler_RTRegNet = get_scheduler(args, optimizer_RTRegNet)

    return optimizer_RTNet, scheduler_RTNet, optimizer_RTRegNet, scheduler_RTRegNet



def train_RTNet(model_RT_relative, model_RT_relative_Reg, optimizer_RTNet, schedular_RTNet, optimizer_RTRegNet, schedular_RTRegNet, numEpochs, model_save_path):
    if args.trainRT_net:
        model_RT_relative.train()
        model_RT_relative_Reg.train()
        # criterion_RT = nn.MSELoss(reduction='mean')
        criterion_RT = nn.L1Loss(reduction='sum')
        print("Training RT Net ..")
    else:
        model_RT_relative.eval()
        model_RT_relative_Reg.eval()
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            x_conv1, x_conv2 = model_RT_relative(ref_imgs.cuda(), src_imgs.cuda())
            ...
            optimizer_RTRegNet.step()
            lrReg = get_learning_rate(optimizer_RTRegNet)




            optimizer_RTRegNet.zero_grad()
            rot_out, tra_out = model_RT_relative_Reg(x_conv1, x_conv2)
            ...
            print('Epoch : {} out of {}, batch : {} out of {}, RT loss : {}, Feat loss : {}, Total loss : {}, Lr_Feat : {}, Lr_Reg : {}'.format(epoch, args.num_epochs,
                                                                                                batch_idx,
                                                                                                len(train_dataloader_),
                                                                                                loss_rtnet, loss_feat, (loss_rtnet+loss_feat), lr, lrReg))
...
def test_RTNet(model_RT_relative, model_RT_relative_Reg, exp_name, start_epoch):
    avgMSEloss_RTnet = 0
    criterion_RT = nn.L1Loss(reduction='sum')
    model_RT_relative.eval()
...
if args.trainRT_net:
    ...
    model_RT_relative_Reg = RegressNet()
    ...
    if (0 and args.train_from_scratch == True):
        ...
        if not os.path.exists(args.modelRT_load_path):
            ...
            print('Loading RT Regressor model from', args.modelRT_Reg_load_path)
            checkpoint = torch.load(args.modelRT_Reg_load_path)
            # model_RT.load_state_dict(checkpoint['model_state_dict'])
            model_RT_relative_Reg.load_state_dict(checkpoint)


    model_RT_relative = model_RT_relative.cuda()
    model_RT_relative_Reg = model_RT_relative_Reg.cuda()
    ...
    optimizer_RTNet, schedular_RTNet, optimizer_RTRegNet, schedular_RTRegNet = configure_optimizers_RTNet(model_RT_relative, model_RT_relative_Reg)

    if not os.path.exists(outdir):
        os.mkdir(outdir)

    train_RTNet(model_RT_relative, model_RT_relative_Reg, optimizer_RTNet, schedular_RTNet,  optimizer_RTRegNet, schedular_RTRegNet, args.num_epochs, model_save_path)
...
#    if not os.path.exists(args.modelMVSNet_save_path):
    ...
    test_RTNet(model_RT_relative, model_RT_relative_Reg, args.exp_name, args.start_epoch)
