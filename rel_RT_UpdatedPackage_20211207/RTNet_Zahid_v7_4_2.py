...
from datasets.dtu_both_relative8_2 import DTUDataset # --> new conversion, includes reading KPS files
...
from models.SqueezeNetHourGlass import SqueezeNetHourGlass
...
args = get_opts()
base_model_RT = models.squeezenet1_0(pretrained=True)
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            for i in range(B_ * V_):
                ...
                ref_kps[:,0] = (ref_kps[:, 0] * (54 / 160))
                ref_kps[:, 1] = (ref_kps[:, 1] * (54 / 128))
                ref_kps = ref_kps.astype(np.int32)
                src_kps = (kps_reshaped[i, :, 2:4])
                src_kps[:, 0] = (src_kps[:, 0] * (54 / 160))
                src_kps[:, 1] = (src_kps[:, 1] * (54 / 128))
...
if args.trainRT_net:
    ...
    #sum_mode = False
    model_RT_relative = SqueezeNetHourGlass(base_model_RT, sum_mode, args.dropout_rate, args.bayesian)
...
if args.testRT_net:
    # initialize RT Net
    model_RT_relative = SqueezeNetHourGlass(base_model_RT, args.sum_mode, args.dropout_rate, args.bayesian)
...
def test_MVSNet(model_RT_relative, model_MVSNet, epoch):
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        #pos_out, ori_out = model(images1)
        B, V, C, H, W = ref_imgs.shape  # Zahid for RT B - batch size, V - no of views
