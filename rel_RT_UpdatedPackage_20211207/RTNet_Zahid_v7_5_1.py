...
from datasets.dtu_both_relative8_2_2 import DTUDataset # --> new conversion, includes reading KPS files
...
def val_dataloader():
    val_dataset = DTUDataset(root_dir=args.root_dir,
                              split='val',
                              n_views=args.n_views,
                              n_depths=args.n_depths,
                              interval_scale=args.interval_scale,
        ...
        for batch_idx, (
            ...
            for i in range(B_ * V_):
                ...
                for j in range(len(ref_kps)): # use only 4 points to compute the loss
                    ...
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1]-1, ref_kps[j, 0]-1]) - (x_conv2[i, :, src_kps[j, 1]-1, src_kps[j, 0]-1])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1]-1, ref_kps[j, 0]]) - (x_conv2[i, :, src_kps[j, 1]-1, src_kps[j, 0]])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1]-1, ref_kps[j, 0]+1]) - (x_conv2[i, :, src_kps[j, 1]-1, src_kps[j, 0]+1])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1], ref_kps[j, 0]-1]) - (x_conv2[i, :, src_kps[j, 1], src_kps[j, 0]-1])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1], ref_kps[j, 0]+1]) - (x_conv2[i, :, src_kps[j, 1], src_kps[j, 0]+1])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1]+1, ref_kps[j, 0]-1]) - (x_conv2[i, :, src_kps[j, 1]+1, src_kps[j, 0]-1])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1]+1, ref_kps[j, 0]]) - (x_conv2[i, :, src_kps[j, 1]+1, src_kps[j, 0]])))
                    loss_feat += torch.sum(abs((x_conv1[i, :, ref_kps[j, 1] + 1, ref_kps[j, 0] + 1]) - (x_conv2[i, :, src_kps[j, 1] + 1, src_kps[j, 0] + 1])))
...
def val_RTNet(model_RT_relative):
    ...
    for batch_idx, (
        ...
        print('batch : {} out of {}, Val loss : {}'.format(batch_idx, len(val_dataloader_), avgMSEloss_RTnet))
