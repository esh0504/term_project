...
from datasets.dataset_aaltoV2 import AALTORelPoseDataset # --> new conversion, includes reading KPS files
...
'''
def val_dataloader():
    val_dataset = AALTORelPoseDataset(root_dir=args.root_dir, split='val', transforms=val_augs)
    if args.num_gpus > 1:
...
def train_dataloader():
    train_dataset = AALTORelPoseDataset(root_dir=args.root_dir, split='train', transforms=train_augs)
    if args.num_gpus > 1:
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for train_sample in tqdm(train_dataloader_):
            ...
            for i in range(B_):
                ...
                ref_kps[:, 0] = (ref_kps[:, 0] * (56 / 1920)) # size of images in aalto university dataset is 1920 x 1080
                ref_kps[:, 1] = (ref_kps[:, 1] * (56 / 1080))
                ref_kps = ref_kps.astype(np.int32)
                src_kps = (kps_reshaped[i, :, 2:4])
                src_kps[:, 0] = (src_kps[:, 0] * (56 / 1920))
                src_kps[:, 1] = (src_kps[:, 1] * (56 / 1080))
                src_kps = src_kps.astype(np.int32)
                # print('points1[{}] : {}'.format(i, ref_kps))
                # print('points2_gt[{}] : {}'.format(i, src_kps))
                # print("len(ref_kps[{}]) = {}, stride = {}".format(i, len(ref_kps[i]), stride))
            ...
            print(
                'Epoch : {} out of {}, RT loss : {}, Feat loss : {}, Total loss : {}, Lr : {}'.format(
        ...
        '''if(loss_rtnet<old_loss):
            ...
            Log.i('Saving models at Epoch : {} out of {}, RT loss : {}, Lr : {}'.format(epoch, args.num_epochs,
                                                                                                    loss_rtnet, lr))'''
