...
def test_dataloader():
    ...
    test_dataset = BlendedMVSDataset(root_dir=args.root_dir, split='test', transforms=None)
    if args.num_gpus > 1:
...
def train_dataloader():
    ...
    train_dataset = BlendedMVSDataset(root_dir=args.root_dir, split='train', transforms=None)
    if args.num_gpus > 1:
...
def test_RTNet(model_RT_relative, exp_name, start_epoch):
    ...
    for train_sample in tqdm(train_dataloader_):
        ...
        with torch.no_grad():
            ...
            rot_out, tra_out, x_conv1, x_conv2 = model_RT_relative(train_sample['img1'].cuda(),
                                                                   train_sample['img2'].cuda())
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(tra_out.shape, rot_out.shape, tra_out, rot_out))
        # print("size of pos_out={}, ori_out={}".format(tra_out,rot_out))
