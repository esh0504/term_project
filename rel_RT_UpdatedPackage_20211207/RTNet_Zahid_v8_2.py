...
from datasets.dtu_both_relative8 import DTUDataset # --> new conversion, includes reading KPS files
#from datasets.dtu_both_relative9 import DTUDataset # --> new conversion, includes reading KPS files, Also, the dataloader uses all the matching combination between two views, no of views will be 2
...
#from models.hourglass_esh_Alex import HourglassNet
...
outdir = "./ckpts/esh_RT/" + args.exp_name + "_" + args.base_model
...
outdir = outdir + "/" + args.loss_method 
print(outdir)
if not os.path.exists(outdir):
    os.mkdir(outdir)
    print(outdir)

outdir = outdir + "/" + "trainResults"
...
LogDir = outdir + "/trainLog_" + str(datetime.datetime.now()) + ".log"
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    if args.trainRT_net:
        ...
        criterion_RT = nn.MSELoss(reduction='sum')
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
                ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, depth, mask, kps_) in enumerate(
                train_dataloader_):
            ...
            # tra_out = F.normalize(tra_out, p=2, dim=1)
            ...
            kps_reshaped = (kps_.cpu().detach().numpy()).reshape(B * V, 3, 4)  # -->relRT36 funetine after 100 epochs
            '''
            ...
            loss_feat = 0
            cnt = 0
            #1for i in range(B_ * V_):
                # here we used a fixed points, rather than taking from SIFT
                #1points1 = kps_reshaped[i,:,:2]
                #1points2_proj_GT = kps_reshaped[i, :, 2:4]
                ...
                # points2_proj_GT = inverse_project(intrinsics, rel_quat_[i, :4], rel_quat_[i, 4:], depth[math.floor(i / V_)],
                                                #   points1)
                #1 points2_proj_pre = inverse_project3(intrinsics, pred[i, :4].cpu().detach(), pred[i, 4:].cpu().detach(),
                #1                                    depth[math.floor(i / V_)],
                #1                                    points1)
            ...
            #1    points2_proj_pre = np.array(points2_proj_pre)
            ...
            #1    loss_feat += np.sum(abs(points2_proj_GT[:, 0] - points2_proj_pre[:, 0]) + abs(points2_proj_GT[:, 1] - points2_proj_pre[:, 1]))
                #print("loss feat :", loss_feat)
                #quit()

                #1 '''for j in range(len(points1)):
                #1     if(points2_proj_GT[j,0]>0 and points2_proj_pre[j,0] >0 and points2_proj_GT[j,1]>0 and points2_proj_pre[j,1]>0 and mask_[points1[j,1], points1[j,0]] > 0\
                #1         and points2_proj_GT[j, 0] < 160 and points2_proj_pre[j, 0] <160 and points2_proj_GT[j, 1] < 128 and points2_proj_pre[j, 1] < 128 ):
                #1        loss_feat += abs(points2_proj_GT[j,0] - points2_proj_pre[j,0]) + abs(points2_proj_GT[j,1] - points2_proj_pre[j,1])
                #1         cnt += 1
                #1        #print("points2_proj_GT[{},0] - points2_proj_pre[{},0] : {}".format(j,j,points2_proj_GT[j,0] - points2_proj_pre[j,0]))
                #1         #print("points2_proj_GT[{},1] - points2_proj_pre[{},1] : {}".format(j,j,points2_proj_GT[j, 1] - points2_proj_pre[j, 1]))
                #1         #print("Loss Feat : {}".format(loss_feat))
            ...
            if (0 and args.usePercenLoss):
                ...
                #print( w1 * criterion_RT(pred[:, 0], rel_quat_[:, 0].cuda()), w2 * criterion_RT(pred[:, 4], rel_quat_[:, 4].cuda()))
            ...
            print('Epoch : {} out of {}, batch : {} out of {}, RT loss : {}, Total loss : {}, Lr : {}'.format(epoch, args.num_epochs,
                                                                                                batch_idx,
                                                                                                len(train_dataloader_),
                                                                                                loss_rtnet,(loss_rtnet+loss_feat), lr))
        ...
        if args.loss_feat:
            if(loss_rtnet+loss_feat < old_loss or epoch % 10 == 0):
                old_loss = loss_rtnet+loss_feat
                save_models(model_RT_relative, epoch, model_save_path)
                Log.i('Saving models at Epoch : {} out of {}, batch : {} out of {}, RT loss : {}, Lr : {}'.format(epoch, args.num_epochs,
                                                                                                        batch_idx,
                                                                                                        len(train_dataloader_),
                                                                                                        loss_rtnet, lr))
            # else:
            ...
            if(loss_rtnet < old_loss or epoch % 10 == 0):
...
def test_RTNet(model_RT, exp_name, start_epoch):
    ...
    criterion_RT = nn.L1Loss(reduction='sum')
    model_RT.eval()
    #model_RT.apply(apply_dropout)
    print("RT Net is used as RT predictor ..")
    
    if not os.path.exists('./' + outdir + '/epoch_' + str(start_epoch)):
        os.mkdir('./' + outdir + '/epoch_' + str(start_epoch))
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        ...
        img_ = (src_imgs[:, :, ::2, ::2]).cpu()  # batch 0, all ref images, 1/4 scale
        #save_image(img_, outdir + '/epoch_' + str(start_epoch) + "/" + "src.png")

        ref_imgs = ref_imgs.reshape(B * V, 3, H, W)  # --> pass the view images as batch images to RT network
        # print("B={}, V={}, C={}, H={}, W={}".format(B, V, C, H, W))

        img_ = (ref_imgs[ :, :, ::2, ::2]).cpu()  # batch 0, all ref images, 1/4 scale
        #save_image(img_, outdir + '/epoch_' + str(start_epoch) + "/" + "ref.png")
        
        src_imgs.cuda()
        with torch.no_grad():
            rot_out, tra_out,_,_ = model_RT(ref_imgs.cuda(), src_imgs.cuda())
        # print("size of pos_out = {}, ori_out = {}, pos_out={}, ori_out={}".format(tra_out.shape, rot_out.shape, tra_out, rot_out))
        # print("size of pos_out={}, ori_out={}".format(tra_out,rot_out))
        ...
        # print("size of gt_qua = {}, gt_qua = {}".format((gt_quaternion[0].shape), torch.cat(gt_quaternion[0], gt_quaternion[1], gt_quaternion[0])))
        rel_quat = rel_quat_.reshape(B * V, 7)
...
if args.trainRT_net:
    ...
    model_save_path = "/data_hdd/SeungHo/model_ckpt/RTNet/" + args.exp_name + "_" + args.base_model
    if args.loss_feat:
        model_save_path += "_feat"
...
def test_MVSNet(model_RT_relative, model_MVSNet, epoch):
    ...
    for batch_idx, (ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth, depth_values, mask, view_ids) in enumerate(test_dataloader_):
        if args.MVSTest > int(filename_to_save_ref[0][0][-14:-12]):
            continue
        ...
        with torch.no_grad():
            if 1: #write outputs
                if os.path.exists('./' + outdir + '/epoch_' + str(epoch) + set):
                    ...
                    print("filename_to_save_ref[0][:-4] : ", filename_to_save_ref[0][:-4] )
                    print("filename_to_save_ref[0][:-4] : ", filename_to_save_ref[0][:-4] )
