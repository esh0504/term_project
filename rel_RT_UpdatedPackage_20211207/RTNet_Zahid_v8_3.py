...
#from datasets.dtu_both_relative8_3 import DTUDataset # --> new conversion, includes reading KPS files
from datasets.dtu_both_relative8_4 import DTUDataset # --> new conversion, includes reading KPS files, uses DPE images and no normalization is applied
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            kps_reshaped = (kps_.cpu().detach().numpy()).reshape(B * V, 4, 4)  # -->relRT36 funetine after 100 epochs
            '''
