...
from datasets.dtu_both_relative8 import DTUDataset # --> new conversion, read the keypoints directly
...
# models
from models.hourglass_Zahidv7 import HourglassNet # for RTNet_Zahidv9 we are using new network, hourglass_zahidv7
#from models.hourglass_Zahidv8 import HourglassNet # for RTNet_Zahidv9 we are using new network, hourglass_zahidv8, updated hourglass_zahidv7 by reducing # of convolutional layers
...
def train_RTNet(model_RT_relative, optimizer_RTNet, schedular_RTNet, numEpochs, model_save_path):
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            rot_out, tra_out, _, _, _, _ = model_RT_relative(ref_imgs.cuda(), src_imgs.cuda())
            ...
            #kps_reshaped = (kps_.cpu().detach().numpy()).reshape(B * V, 20, 4)# --> relRT35
            kps_reshaped = (kps_.cpu().detach().numpy()).reshape(B * V, 2, 4)# --> relRT37
            '''
            ...
            for i in range(B_ * V_):
                ...
                loss_feat += np.sum(abs(points2_proj_GT[:, 0] - points2_proj_pre[:, 0]) + abs(
                    points2_proj_GT[:, 1] - points2_proj_pre[:, 1]))
            ...
            if (0 and args.usePercenLoss):
                ...
                loss_rtnet = perc_loss(pred,
                                       rel_quat_)  # --> fintuning the relRT19 experiment, checkpoints will be 100~200
            else:
                loss_rtnet = w1 * criterion_RT(pred[:, 0], rel_quat_[:, 0].cuda()) + w1 * criterion_RT(pred[:, 1],
                    ...
                    pred[:, 6], rel_quat_[:,
                                6].cuda())  # --> for relRT19, we used this loss as loss_rtnet, and after 100 epochs, we fine tune using perc_loss
