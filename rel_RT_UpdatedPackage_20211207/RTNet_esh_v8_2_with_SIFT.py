...
from datasets.dtu_both_relative8_withSift import DTUDataset # --> new conversion, includes reading KPS files
...
outdir = "./ckpts/esh_RT_SIFT/" + args.exp_name + "/"

if not os.path.exists(outdir):
    os.mkdir(outdir)

outdir = "./ckpts/esh_RT_SIFT/" + args.exp_name + "/" + "trainResults"
...
LogDir = "./ckpts/esh_RT_SIFT/" + args.exp_name + "/trainLog_" + str(datetime.datetime.now()) + ".log"
    ...
    for epoch in range(args.start_epoch, numEpochs + 1):
        ...
        for batch_idx, (
            ...
            for i in range(B_ * V_):
                # here we used a fixed points, rather than taking from SURF
                ...
                '''points2_proj_GT = inverse_project(intrinsics, rel_quat_[i, :4], rel_quat_[i, 4:], depth[math.floor(i / V_)],
                                                  points1)'''
                points2_proj_pre = inverse_project4(intrinsics, pred[i, :4].cpu().detach(), pred[i, 4:].cpu().detach(),
                                                   points1)
...
if args.trainRT_net:
    ...
    model_save_path = "./ckpts/esh_RT_SIFT/" + args.exp_name
