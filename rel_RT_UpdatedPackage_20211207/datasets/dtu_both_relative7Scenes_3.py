...
class SevenScenes(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            newFolder = self.root_dir.replace('7Scenes', '7Scenes/7Scenes_DPE')
            img_filename = os.path.join(newFolder,f'{scan}/frame-{vid:06d}.color.jpg')
            ...
            if i == 0:  # reference view
                ...
                temp_kps = this_kps[np.arange(0, this_kps.shape[0], int(np.floor((this_kps.shape[0]) / 16))), :]
                if(temp_kps.shape[0] > 16):
                    kps_ += [torch.FloatTensor(temp_kps[:16,:])]  # --> all keypoints used for calculating the conv layer based loss
                else:
