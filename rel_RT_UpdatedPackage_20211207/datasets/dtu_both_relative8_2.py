    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                r = R_.from_matrix(extrinsics_[:3, :3])
                qvec = r.as_quat()
                x = float(T_temp_[0]) / 1000.0
                y = float(T_temp_[1]) / 1000.0
                z = float(T_temp_[2]) / 1000.0 # --> dividing by 1000 to reduce the error unit and also get rid of the scale factor term in the loss equation
                # print("extrinsics after quat indx :{} = {}".format(vid, extrinsics))
                #print("x={}, y={},z={}".format(x,y,z))
                q1 = float(qvec[3])
                q2 = float(qvec[0])
        ...
        if (self.split == 'train'):
            ...
            return ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, kps_  # when training relative RT net only
        ...
        '''if (self.split == 'test'):
            ...
            return ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, imgs, depth_values, mask, view_ids_  # when training and testing relative RTNet with MVSNET'''
