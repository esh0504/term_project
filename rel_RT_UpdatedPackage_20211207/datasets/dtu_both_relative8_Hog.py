...
class DTUDataset(Dataset):
    ...
    def Get_HOG_Descriptor(self, image):
        ...
        H = hog(resize_img, orientations=9, pixels_per_cell=(16, 16),cells_per_block=(2, 2), transform_sqrt=True, block_norm="L1", visualize=False)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                ref_hog_descriptor = self.Get_HOG_Descriptor(img)  # compute the HoG features before transforming the image
            else:
                ...
                temp_kps = this_kps[np.arange(0, this_kps.shape[0], int(np.floor((this_kps.shape[0]) / 2))), :]
                if(temp_kps.shape[0] > 2):
                    kps_ += [torch.FloatTensor(temp_kps[:2,:])]  # --> all keypoints used for calculating the conv layer based loss
                else:
                ...
                hog_descriptor = self.Get_HOG_Descriptor(img)
                HoG_feat += [torch.cat((hog_descriptor,ref_hog_descriptor), -1)]
        ...
        if (self.split == 'train'):
            ...
            return ref_imgs, src_imgs, rel_quat_, filename_to_save_ref, filename_to_save_src, depth, kps_, HoG_feat  # when training relative RT net only
