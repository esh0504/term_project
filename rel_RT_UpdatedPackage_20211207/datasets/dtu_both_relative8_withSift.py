...
class DTUDataset(Dataset):
    ...
    def build_metas(self):
        ...
        #pair_file = "Cameras/pair_zahid2.txt" #all 1x49 combinations # testing MVSNEt
        pair_file = "Cameras/pair_zahid4.txt"  # all 49x49 combinations #training RTNEt, 8 views per line
        for scan in scans:
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                if (self.split == 'train'):
                    if(ref_id_ == vid):
                        kps_ += [torch.FloatTensor(np.array([[50,50, 50,50],[52,52, 52,52],[55,55, 55,55]]))]
                        #print('KPS', torch.FloatTensor(np.array([[50,50, 50,50],[52,52, 52,52],[55,55, 55,55]])))
                        #print('KPS size', (torch.FloatTensor(np.array([[50,50, 50,50],[52,52, 52,52],[55,55, 55,55]]))).shape)
                    else:
                        kp_filename = './esh_keypoints_with_SIFT_ver1.1/' + f'{scan}_train/points_{ref_id_+1:02d}_{vid+1:02d}.txt'
                        try:
                            ...
                            temp_kps = this_kps[np.arange(0, this_kps.shape[0], int(np.floor((this_kps.shape[0]) / 3))), :]
                            #print('KPS', temp_kps[:3,:])
                            #print('KPS size', (temp_kps[:3, :]).shape)
                            if(temp_kps.shape[0] > 3):
                                kps_ += [torch.FloatTensor(temp_kps[:3,:])]  # --> all keypoints used for calculating the conv layer based loss
                            else:
        ...
        if (self.split == 'test'): #uncomment this if you want to use RTNet for inference (3D reconstruction)
        ...
        if (self.split == 'test'): #uncomment this if you want to use RTNet for performance evaluation
