...
class SqueezeNetHourGlass(nn.Module):
    def __init__(self, base_model, sum_mode=False, dropout_rate=0.0, bayesian=False):
        ...
        # Encoding Blocks
        self.init_block = nn.Sequential(base_model.features[:3])

        self.res_block1 = nn.Sequential(base_model.features[3:5])
        self.res_block2 = nn.Sequential(base_model.features[5:7])
        self.res_block3 = nn.Sequential(base_model.features[7:12])
        self.res_block4 = nn.Sequential(base_model.features[12])


        # Decoding Blocks
        if sum_mode:
            self.deconv_block1 = nn.ConvTranspose2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
            self.deconv_block2 = nn.ConvTranspose2d(512, 256, kernel_size=(3, 3), stride=(2, 2), padding=(0, 0), bias=False, output_padding=0)
            self.deconv_block3 = nn.ConvTranspose2d(256, 128, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False, output_padding=1)
            self.deconv_block4 = nn.ConvTranspose2d(128, 96, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1),bias=False)
            self.conv_block = nn.Conv2d(96, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
            print("This")
        else:
            self.deconv_block1 = nn.ConvTranspose2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
            self.deconv_block2 = nn.ConvTranspose2d(512, 256, kernel_size=(3, 3), stride=(2, 2), padding=(0, 0), bias=False, output_padding=0)
        ...
        # Regressor
        self.fc_dim_reduce = nn.Linear(54 * 54 * 32, 1024)
    ...
    def forward_one(self, x):
        ...
        #print('x_dconv1 = {}'.format(x_deconv1.size()))
        if self.sum_mode:
        ...
        #print('x_dconv2 = {}'.format(x_deconv2.size()))
        if self.sum_mode:
        ...
        #print('x_dconv3 = {}'.format(x_deconv3.size()))
        if self.sum_mode:
            x_deconv3 = x_res1 + x_deconv3
        else:
            x_deconv3 = torch.cat((x_res1, x_deconv3), dim=1)

        x_deconv4 = self.deconv_block4(x_deconv3)
        #print('x_dconv4 = {}'.format(x_deconv4.size()))
        if self.sum_mode:
            x_deconv4 = x + x_deconv4
        else:
            x_deconv4 = torch.cat((x, x_deconv4), dim=1)

        x_conv = self.conv_block(x_deconv4)
