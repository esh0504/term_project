...
class HourglassNet(nn.Module):
    def __init__(self, base_model, sum_mode=False, dropout_rate=0.0, bayesian=False):
        ...
        # Regressor
        #self.fc_dim_reduce = nn.Linear((56 * 56 * 32) + 26244, 4096)
        #self.fc_dim_reduce = nn.Linear((56 * 56 * 32) + 6084, 4096)
        self.fc_dim_reduce = nn.Linear((56 * 56 * 32), 1024)
        self.fc_dim_reduce1 = nn.Linear(6084*2, 4096)
        self.fc_dim_reduce2 = nn.Linear(4096, 2048)
        self.fc_dim_reduce3 = nn.Linear(2048, 1024)
    ...
    def forward_one(self, x):
        # Conv
        #print("Shape of feature in network Forward 1 : {}".format(HoG_feat.shape))
        x = self.init_block(x)
        ...
        x_linear_ = x_conv.view(x_conv.size(0), -1)
        #x_linear_ = torch.cat((x_linear, HoG_feat), 1)
        x_linear = self.fc_dim_reduce(x_linear_)
    ...
    def forward(self, x1, x2, HoG_feat):
        HoG_feat_ = self.fc_dim_reduce1(HoG_feat)
        HoG_feat_ = F.relu(HoG_feat_)
        HoG_feat_ = self.fc_dim_reduce2(HoG_feat_)
        HoG_feat_ = F.relu(HoG_feat_)
        HoG_feat_ = self.fc_dim_reduce3(HoG_feat_)
        HoG_feat_ = F.relu(HoG_feat_)
        #print("Shape of feature in network Forward : {}".format(HoG_feat.shape))
        #print("Shape of image in network Forward : {}".format(x1.shape))
        #x1_, x_conv1 = self.forward_one(x1, HoG_feat[:,:26244])
        #x1_, x_conv1 = self.forward_one(x1, HoG_feat[:, :6084])
        x1_, x_conv1 = self.forward_one(x1)
        #x1_ = self.forward_one(x1)
        #x2_, x_conv2 = self.forward_one(x2, HoG_feat[:,26244:])
        #x2_, x_conv2 = self.forward_one(x2, HoG_feat[:, 6084:])
        x2_, x_conv2 = self.forward_one(x2)
        #x2_ = self.forward_one(x2)
        diff = torch.abs(x1_ - x2_)
        out = torch.cat((x1_, x2_, diff, HoG_feat_), 1)
