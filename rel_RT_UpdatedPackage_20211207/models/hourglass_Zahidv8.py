...
class HourglassNet(nn.Module):
    def __init__(self, base_model, sum_mode=False, dropout_rate=0.0, bayesian=False):
        ...
        self.conv_block_combine = nn.Conv2d(32*2, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False) # feature size 56 x 56 x 32
        self.conv_block_dimRed1 = nn.Conv2d(32, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False) # feature size 28 x 28 x 32
        self.conv_block_dimRed2 = nn.Conv2d(32, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False) # feature size 14 x 14 x 32
        #self.conv_block_dimRed3 = nn.Conv2d(32, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False) # feature size 7 x 7 x 32
        ...
        #self.fc_dim_reduce2 = nn.Linear(7 * 7 * 32, 1024)
        self.fc_dim_reduce2 = nn.Linear(14 * 14 * 32, 1024)
        ...
        init_modules = [self.deconv_block1, self.deconv_block2, self.deconv_block3, self.conv_block,
                        self.conv_block_combine, self.conv_block_dimRed1, self.conv_block_dimRed2, self.fc_dim_reduce2, self.FinalLayer_rot, self.FinalLayer_tra]
    ...
    def convolve_siamese_outs(self, x1, x2):
        x = torch.cat([x1,x2],1) # 56 x 56 x 64
        y = self.conv_block_combine(x) # 56 x 56 x 32
        y = self.conv_block_dimRed1(y) # 28 x 28 x 32
        y = self.conv_block_dimRed2(y) # 14 x 14 x 32
        #y = self.conv_block_dimRed3(y) # 7 x 7 x 32
        return y

    def forward(self, x1, x2):
        x_conv1 = self.forward_one(x1)
        #x1_ = self.forward_one(x1)
        x_conv2 = self.forward_one(x2)
        #conv_out = self.convolve_siamese_outs(x_conv1, x_conv2) #--> 7 x 7 x 32
        conv_out = self.convolve_siamese_outs(x_conv1, x_conv2)  # --> 14 x 14 x 32
        conv_out2 = self.makeItFlat(conv_out) #--> size 1x1024

        #x2_ = self.forward_one(x2)
        #diff = torch.abs(x1_ - x2_)
        #out = torch.cat((x1_, x2_, diff), 1)
        rot = self.FinalLayer_rot(conv_out2)
        trans = self.FinalLayer_tra(conv_out2)
        #return rot, trans, x_conv1, x_conv2, x1_, x2_
        return rot, trans, x_conv1, x_conv2, conv_out, conv_out2
        #return rot, trans